            ...
            <ul class="vertical-nav-menu">
                ...
                <li>
                    ...
                    <ul>
                        ...
                        $selExmneeData = $conn->query("SELECT * FROM examinee_course_tbl WHERE examinee_id='$exmneId' ");

                        if ($selExmneeData->rowCount() > 0) {
                            $flag = 0;
                            while ($selExam = $selExmneeData->fetch(PDO::FETCH_ASSOC)) {
                                $courseId = $selExam['cou_id'];
                                $selExam = $conn->query("SELECT * FROM exam_tbl WHERE cou_id='$courseId' ORDER BY ex_id DESC ");
                                while ($selExamRow = $selExam->fetch(PDO::FETCH_ASSOC)) {
                                    ?>
                                    <?php if ($selExamRow['published'] == 1) {
                                        $flag = 1; ?>
                                        <li>
